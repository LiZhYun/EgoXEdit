...
📁 Data Sources:
- HDF5 files: Generated by /home/zhiyuan/Codes/human-policy/data/save_object_trajectory.py
- Dataset path: /home/zhiyuan/Codes/DataSets/small_test
- Expected data: dual-hand motion sequences (20D), object trajectories, end-effector images

🤖 Dual-Hand Motion Format (NEW):
...
🔧 Compared to original Wan2.1-VACE-1.3B.py:
...
def generate_task_prompt(task_name, metadata):
    ...
    if not task_attrs:
        print(f"⚠️ Task {task_name} not found in metadata, using fallback prompt")
        return "Robot manipulation task with hand movements and object interactions."
...
def load_robot_data_from_hdf5(dataset_path="/home/zhiyuan/Codes/DataSets/small_test"):
    ...
    dataset_path/
    └── task_folder/ (e.g., 104-lars-grasping_2024-11-08_15-23-40)
        ├── episode_0/
        │   ├── episode_0_hand_trajectories.hdf5
        │   ├── episode_0_object_trajectories.hdf5
        │   ├── episode_0_hands_masked.mp4
        │   └── episode_0_hands_mask.mp4
        ├── episode_1/
        │   ├── episode_1_hand_trajectories.hdf5
        │   ├── episode_1_object_trajectories.hdf5
        │   ├── episode_1_hands_masked.mp4
        │   └── episode_1_hands_mask.mp4
        └── ...
    ...
    if not task_folders:
        print(f"⚠️ No task folders found in {dataset_path}")
        return []
    ...
    for task_folder in task_folders:
        ...
        if not episode_folders:
            print(f"  ⚠️ No episode folders found in {task_path}")
            continue
        ...
        for episode_folder in episode_folders:
                ...
                try:
                    ...
                    with h5py.File(hand_hdf5_path, 'r') as f:
                        ...
                        if 'left_wrist' in f and 'right_wrist' in f:
                            ...
                            print(f"      ✓ Left hand - Position: {left_positions.shape}, Rotation: {left_rotations.shape}")
                            print(f"      ✓ Right hand - Position: {right_positions.shape}, Rotation: {right_rotations.shape}")
                            ...
                            if 'left_hand_states' in f:
                                ...
                                print(f"      ✓ Left gripper states loaded: unique values {np.unique(left_gripper_states)}")
                            else:
                                print(f"      ⚠️ No left_hand_states found, using default (closed)")
                            ...
                            if 'right_hand_states' in f:
                                ...
                                print(f"      ✓ Right gripper states loaded: unique values {np.unique(right_gripper_states)}")
                            else:
                                print(f"      ⚠️ No right_hand_states found, using default (closed)")
                            ...
                            print(f"      ⚠️ Only right hand data found, creating dual-hand with left=zeros")
                            ...
                            print(f"      ⚠️ No valid wrist data found in HDF5 file")
                        
                except Exception as e:
                    print(f"      ⚠️ Error loading hand trajectories: {e}")
            ...
            if episode_data['vace_video_path']:
                print(f"      ✓ VACE video: {os.path.basename(episode_data['vace_video_path'])}")
            else:
                print(f"      ⚠️ VACE video not found: {episode_name}_hands_masked.mp4")
                
            if episode_data['vace_mask_path']:
                print(f"      ✓ VACE mask: {os.path.basename(episode_data['vace_mask_path'])}")
            else:
                print(f"      ⚠️ VACE mask not found: {episode_name}_hands_mask.mp4")
